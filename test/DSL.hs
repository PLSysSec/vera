...
fpTest = benchTestCase "fp" $ do
  ...
  r <- D.evalVerif Nothing $ do
    doubSort <- M.mkDoubleSort
    notInf <- M.mkFpFromDouble (-510423550381407695195061911147652317184) doubSort
    D.doubv "not inf" >>= D.assign notInf

    itIsInf <- D.isInf notInf
    D.i1v "is inf" >>= D.assign itIsInf
    -- doubSort <- M.mkDoubleSort
